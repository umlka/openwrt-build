...
### 通用补丁 ###
# lrng
cp -f ../patch/lrng/* target/linux/generic/hack-6.1/
cat ../config/x86/lrng >> target/linux/generic/config-6.1
# wireguard
cp -f ../patch/wireguard/950-wireguard-support-hotplug.patch target/linux/generic/hack-6.1/
# odhcpd
...
cp -f ../lean_lede/target/linux/generic/hack-6.1/952-add-net-conntrack-events-support-multiple-registrant.patch target/linux/generic/hack-6.1/
# threaded network backlog processing
rm -f target/linux/generic/pending-6.1/760-net-core-add-optional-threading-for-backlog-processi.patch
